...
void checkLight() {
    ...
    if (avarageLL >= QUINZE_PORCENTO && avarageLL <= VINTE_CINCO_PORCENTO) {
        // O nível de luz está dentro da faixa aceitável
        okLight();
    }
    else if ((avarageLL > DEZ_PORCENTO && avarageLL < QUINZE_PORCENTO) ||
             (avarageLL > VINTE_CINCO_PORCENTO && avarageLL < TRINTA_PORCENTO)){
...
void avarageLight(){
    // Mede a média do nível de luz por 5 segundos
    // all = avarage light level
    avarageLL = analogRead(LDR);
    delay(500);
    for(x = 0, x <= 9, x ++ ){
        avarageLL = analogRead(LDR) + avarageLL;
        delay(500)
    }
}